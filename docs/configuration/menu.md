...
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `call`         | The `call` menu button starts doorbell-style call mode for the active camera. It only appears when the active camera has [`call_mode`](./cameras/call-mode.md) enabled, `call_mode.show_in_menu` is `true`, no call is already active, and the current live view is not using a substream override.                                                                                                                                                                                                   |
...
| `enabled`   | `true` for `call`, `iris`, `cameras`, `substreams`, `live`, `clips`, `snapshots`, `timeline`, `download`, `camera_ui`, `fullscreen`, `media_player`, `display_mode` and `ptz_home`. `false` for `image`, `expand`, `microphone`, `mute`, `play`, `recordings`, `screenshot`, `ptz_controls` | Whether or not to show the button.                                                                                                                                                                                                                                                                                           |
