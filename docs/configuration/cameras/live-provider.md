...
| `modes`                          | `[webrtc, mse, mp4, mjpeg]` for `go2rtc-experimental`, `[webrtc, mse, hls, mjpeg]` for `go2rtc`                                                      | An ordered list of `go2rtc` modes to use. Valid values are `webrtc`, `mse`, `mp4` or `mjpeg` values. The `go2rtc` live provider client also falls back to `hls`, which cannot be selected here. |
