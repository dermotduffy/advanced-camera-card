...
| `client_id`   | Autodetected from `camera_entity` if that is specified, otherwise `frigate`. | The Frigate client id to use. Autodetection needs the `camera_entity` to have a state in Home Assistant; if it does not yet (e.g. the Frigate integration is restarting), the default is used and detection is retried. If this Home Assistant server has multiple Frigate server backends configured, this selects which server should be used. It should be set to the MQTT client id configured for this server, see [Frigate Integration Multiple Instance Support](https://docs.frigate.video/integrations/home-assistant//#multiple-instance-support). |
